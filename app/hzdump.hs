...
data Option
    = Version
    | Illegal Char
    | Verbose (Maybe Integer, Maybe Integer)
    | Now
...
main = do
    ...
    putStr . unlines . concat $ zipWith (displayZone getTimes displayTime) zones tzss
  where
    getZone "-" = return utcTZ -- sic
    getZone z = getTimeZoneSeriesFromOlsonFile z
    displayZone getTimes displayTime zone tzs = map ((zone ++) . ("  " ++) . displayTime tzs) $ getTimes tzs

parseArgs :: [String] -> (Option, [FilePath])
parseArgs args
    | "--version" `elem` args = (Version, [])
parseArgs args = getOpts False (Nothing, Nothing) args
  where
    getOpts _ cutoff ("-v":args') = getOpts True cutoff args'
    getOpts v _ ("-c":c:args') = maybe (Illegal 'c', []) (\y -> getOpts v y args') $ parseCutoff c
...
displayConcise tzs t = formatTime defaultTimeLocale format $ ZoneSeriesTime t tzs
  where
    format = "%a %b %e %T %Y %Z"

displayVerbose :: TimeZoneSeries -> UTCTime -> String
displayVerbose tzs t =
    concat
        [ displayConcise utcTZ t
        , " = "
        , displayConcise tzs t
        , " isdst="
        , if isdst
              then "1"
              else "0"
...
transitionTimes :: (Maybe Integer, Maybe Integer) -> TimeZoneSeries -> [UTCTime]
transitionTimes (lo, hi) =
    maybe id (takeWhile . (>)) (fmap happyNewYear hi) .
    maybe id (dropWhile . (>)) (fmap happyNewYear lo) . addBugs . addPrevSecond . actualTransitions
  where
    happyNewYear y = UTCTime (fromGregorian y 1 1) 0
    actualTransitions (TimeZoneSeries d cs) = let
        rs = reverse cs
        in map snd . filter fst $ zipWith (\(t, tz) prevTz -> (tz /= prevTz, t)) rs (d : map snd rs)
    addPrevSecond = concatMap (\t -> [(-1) `addUTCTime` t, t])
    addBugs = (bug0 ++) . (++ bug1) -- sic
    bug0 =
        [ UTCTime (fromGregorian 1901 12 13) (20 * 3600 + 45 * 60 + 52)
        , UTCTime (fromGregorian 1901 12 14) (20 * 3600 + 45 * 60 + 52)
        ]
    bug1 =
        [ UTCTime (fromGregorian 2038 1 18) (3 * 3600 + 14 * 60 + 7)
