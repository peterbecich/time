...
parseNISTLeapSecondList text = do
    let
        readLine ('#':'$':s)
            | Just version <- ntpDateStringToDay s = return (Just version, Nothing, Nothing)
        readLine ('#':'@':s)
            | Just expiration <- ntpDateStringToDay s = return (Nothing, Just expiration, Nothing)
        readLine ('#':_) = return (Nothing, Nothing, Nothing)
        readLine "" = return (Nothing, Nothing, Nothing)
        readLine s =
            case separate '\t' s of
                (tstr:ostr:_)
                    | Just t <- ntpDateStringToDay tstr
                    , Just offset <- readMaybe ostr -> return $ (Nothing, Nothing, Just (t, offset))
                _ -> Nothing
    ...
    let transitions = catMaybes $ fmap (\(_, _, x) -> x) mstrs
...
leapSecondListToMap lsl day
    | day >= lslExpiration lsl = Nothing
