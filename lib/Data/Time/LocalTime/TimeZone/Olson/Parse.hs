...
import Control.Exception.Extensible (ErrorCall, Exception, throw, try)
import Control.Monad (guard, replicateM, replicateM_, when)
import Data.Binary.Get (Get, getByteString, getRemainingLazyByteString, getWord32be, getWord64be, getWord8, runGet)
...
olsonToTimeZoneSeries (OlsonData ttimes ttinfos@(dflt0:_) _ _) =
  ...
  where
    ...
    mkTZ (TtInfo gmtoff isdst _ abbr) = TimeZone ((gmtoff + 30) `div` 60) isdst abbr
    lookupTZ tti ttime = fmap (((,) $ toUTC ttime) . mkTZ) . listToMaybe $ drop (transIndex ttime) tti
...
getOlsonPart :: Integral a => Bool -> SizeLimits -> Get a -> Get (Word8, OlsonData)
getOlsonPart verifyMagic limits getTime = do
    ...
    tzh_leapcnt <- get32bitInt >>= verify (withinLimit maxLeaps) "too many leap second specifications"
    tzh_timecnt <- get32bitInt >>= verify (withinLimit maxTimes) "too many timezone transitions"
    tzh_typecnt <- get32bitInt >>= verify (withinLimit maxTypes) "too many timezone type specifications"
    _ <- verify (withinLimit maxTypes) "too many isgmt specifiers" tzh_ttisgmtcnt
    _ <- verify (withinLimit maxTypes) "too many isstd specifiers" tzh_ttisstdcnt
    tzh_charcnt <- get32bitInt >>= verify (withinLimit maxAbbrChars) "too many tilezone specifiers"
  ...
  where
    ...
    lookupAbbr (TtInfo gmtoff isdst ttype abbrind) = TtInfo gmtoff isdst ttype . takeWhile (/= '\NUL') . drop abbrind
    setTtype ttinfo ttype = ttinfo {tt_ttype = ttype}
    boolsToTType _ isgmt
        | isgmt = UTC
...
getPosixTZ = do
    _ <- getWord8 >>= verify (== 10) "POSIX TZ string not preceded by newline"
...
throwOlson fp msg = throw . OlsonError $ fp ++ ": invalid timezone file: " ++ msg
