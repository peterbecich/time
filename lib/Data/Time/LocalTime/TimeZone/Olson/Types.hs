...
instance Semigroup OlsonData where
    OlsonData a b c d <> OlsonData a' b' c' d' =
        OlsonData (a ++ map (shiftBy $ length b) a') (b ++ b') (c ++ c') (d `mplus` d')
      where
...
defaultLimits = SizeLimits {maxTimes = Just 1200, maxTypes = Just 256, maxAbbrChars = Just 50, maxLeaps = Just 50}
